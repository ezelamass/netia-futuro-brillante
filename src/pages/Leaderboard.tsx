...
const Leaderboard = () => {
  ...
  const pointsToTop3 = currentUser.rank > 3 ? leaderboardData[2].points - currentUser.points + 1 : 0;
  return <AppLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        y: 0
          ...
          delay: 0.4
            ...
            {pointsToTop3 > 0 && <div className="glass-dark rounded-xl px-4 py-2 flex items-center gap-2">
        ...
        {/* Empty state */}
        {!isLoading && !hasEnoughUsers && <Card>
            <CardContent className="p-6">
              <EmptyState variant="no-leaderboard" onAction={() => navigate('/training')} />
            </CardContent>
          </Card>}
        ...
        {!isLoading && hasEnoughUsers && <>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="space-y-2">
                  {restOfList.map((athlete, index) => <motion.div key={athlete.rank} initial={{
                opacity: 0,
                x: 20
              }} animate={{
                opacity: 1,
                ...
                delay: 0.4 + index * 0.1
              }} className={`flex items-center gap-4 p-3 rounded-xl transition-colors ${athlete.name === currentUser.name ? 'bg-primary/10 border-2 border-primary/30' : 'bg-muted/50 hover:bg-muted'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-sm truncate">
                          {athlete.name}
                          {athlete.name === currentUser.name && <span className="ml-2 text-xs text-primary font-normal">(Tú)</span>}
                        </p>
          ...
          delay: 0.6
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {mockPrizes.map((prize, index) => <PrizeCard key={prize.id} position={index + 1 as 1 | 2 | 3} prize={prize} delay={0.7 + index * 0.1} onClick={() => setSelectedPrize({
              prize,
        ...
        {/* Prize Detail Modal */}
        <PrizeDetailModal prize={selectedPrize?.prize ?? null} position={selectedPrize?.position ?? 1} open={!!selectedPrize} onClose={() => setSelectedPrize(null)} />
      </div>
    </AppLayout>;
};
